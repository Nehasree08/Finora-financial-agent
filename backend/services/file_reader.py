    ...
    for engine in engines:
        try:
            ...
            for sheet in workbook.sheet_names:
                ...
                tables.append(LoadedTable(sheet_name=str(sheet), dataframe=df))
            return tables
...
def _usable_dataframe(df: pd.DataFrame) -> bool:
    ...
    for column in df.columns:
        values = df[column]
        if values.notna().any() and not values.astype(str).str.strip().eq("").all():
            return True
    return False
...
def read_uploaded_file(filename: str, content: bytes) -> LoadedFile:
    ...
    if len(content) > MAX_UPLOAD_BYTES:
        raise AnalysisError("Uploaded file is too large. Maximum size is 25 MB.")

    ext = _extension(filename)
    if ext not in SUPPORTED_EXTENSIONS:
        raise AnalysisError("Unsupported file type. Upload a CSV or Excel file.")

    notes: list[str] = []
    if ext == ".csv":
        tables = [LoadedTable(sheet_name="", dataframe=_read_csv(content))]
        file_type = "csv"
    else:
        tables = _read_excel(content, ext)
        file_type = ext[1:]

    usable_tables = [t for t in tables if _usable_dataframe(t.dataframe)]
    if not usable_tables:
        raise AnalysisError("No usable rows and columns were found in the uploaded file.")
    if len(usable_tables) != len(tables):
        notes.append(f"Ignored {len(tables) - len(usable_tables)} empty worksheet(s).")
